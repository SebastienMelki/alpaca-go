...
func (x *CryptoBarsList) UnmarshalJSON(data []byte) error {
	var itemsRaw []json.RawMessage
	if err := json.Unmarshal(data, &itemsRaw); err != nil {
		return err
	}
	x.Bars = make([]*CryptoBar, 0, len(itemsRaw))
...
func (x *GetCryptoBarsResponse) UnmarshalJSON(data []byte) error {
	...
	if rawField, ok := raw["bars"]; ok {
		...
		for k, arrayRaw := range mapRaw {
			...
			x.Bars[k] = &CryptoBarsList{Bars: items}
		}
	}
